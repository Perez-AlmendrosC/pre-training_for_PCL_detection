...
def test_predictions(finetuned_model, test_dataloader, fold_counter):
  ...
  #with open(os.path.join('results',f'results_BASELINE_10epochs_categories_2knegs.txt'), 'a')as outf:
  with open(os.path.join('results',f'results_adapter={args.adapter_name}_categories.txt'), 'a')as outf:
...
# Concatenate categories and negative dataframes into one
  ...
  for s in seed:
    ...
    for fold, (train_ids, test_ids) in enumerate(kf.split(positive_samples)):
      ...
      # Create validation data (10% from training set)
      ...
      train_dataloader = torch.utils.data.DataLoader(
                        t, #t if using validation, train_dataset if not using validation, #train_downsampled_data
      ...
      ft_model=finetune_model(model, train_dataloader, validation_daaloader, epochs=EPOCHS)
